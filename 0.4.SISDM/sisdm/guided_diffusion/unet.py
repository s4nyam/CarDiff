...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.num_classes is not None:
            expected_shape = (x.shape[0], self.num_classes, x.shape[2], x.shape[3])
            assert y.shape == expected_shape, (
                f"Conditioning tensor shape {y.shape} does not match expected"
                f" {expected_shape} for num_classes={self.num_classes}."
            )
