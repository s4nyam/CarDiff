    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if config.class_conditional:
                ...
                # Use explicit keyword arguments for UNet2DModel to avoid DataParallel argument unpack edge cases
                noise_pred = model(sample=noisy_images, timestep=timesteps, class_labels=class_labels, return_dict=False)[0]
            else:
        ...
        if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
            ...
            # Save with epoch number appended
            epoch_specific_dir = os.path.join(config.output_dir, f"checkpoint_epoch_{epoch + 1}")
            os.makedirs(epoch_specific_dir, exist_ok=True)  # Create the directory if it doesn't exist
            pipeline.save_pretrained(epoch_specific_dir)
