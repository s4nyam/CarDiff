#!/usr/bin/env python3
"""
Fill Mask Script for Segmentation-Guided Diffusion Model (Folder-wise)
Generates images conditioned on input masks from multiple subdirectories 
...
import random
import cv2
import re  # Added for epoch extraction
...
def load_masks_dataset(masks_dir):
    ...
    if not mask_files:
        # Return None if no masks are found, so we can skip the folder.
        return None
...
def generate_images_from_masks(model_dir, masks_dir, output_dir="generated_images", 
    ...
    if dataset is None:
        print(f"No masks found in {masks_dir}. Skipping.")
        return 0
...
def main():
    parser = argparse.ArgumentParser(description="Generate images from masks in subfolders using a trained diffusion model")
    
    parser.add_argument("--model_dir", type=str, default="ddim-dc-384-segguided/checkpoint_epoch_1000",
                       help="Path to model checkpoint directory (e.g., /path/to/ddim-dc-384-segguided/checkpoint_epoch_540)")
    
    parser.add_argument("--base_masks_dir", type=str, default="/scratch/project_465001696/playground/0.3.segguideddiff/test_labels_class_wise_ddim",
                       help="Path to the base directory containing subdirectories of mask images.")
    ...
    # Extract epoch number from model_dir (e.g., "checkpoint_epoch_540" -> 540)
    epoch_match = re.search(r'epoch[_-]?(\d+)', args.model_dir)
    if epoch_match:
        epoch_num = epoch_match.group(1)
    else:
        raise ValueError(f"Could not extract epoch number from model_dir: {args.model_dir}. "
                         "Expected pattern like 'checkpoint_epoch_540' or similar.")

    try:
        # Get all items in the base directory
        subdirs = [
            d for d in os.listdir(args.base_masks_dir)
            if os.path.isdir(os.path.join(args.base_masks_dir, d))
        ]

        # Only keep ORIGINAL folders (no "-syn<number>" at the end)
        dirs_to_process = [
            d for d in subdirs
            if not re.search(r"-syn\d+$", d)
        ]
        ...
        for folder_name in dirs_to_process:
            masks_dir = os.path.join(args.base_masks_dir, folder_name)
            output_dir = os.path.join(args.base_masks_dir, f"{folder_name}-syn{epoch_num}")
            
            print("-" * 80)
            print(f"Processing folder: {masks_dir}")
            ...
            print(f"Successfully completed folder {folder_name}! Generated {total_generated} images.")
