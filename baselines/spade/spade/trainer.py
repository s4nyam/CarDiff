...
class SPADETrainer:
    def __init__(self, config: TrainingConfig, dataset: 'SegmentationImageDataset') -> None:
        ...
        # These are used purely for resume logic
        self.global_step = 0
        self._ckpt_epoch = 0  # epoch stored in checkpoint (informational)
    ...
    def _load_checkpoint(self, path: Path) -> None:
        # NOTE: keep weights_only=False for backward compatibility with current checkpoints.
        ...
        self._ckpt_epoch = int(checkpoint.get('epoch', 0))

        self._log(
            f"Resumed from checkpoint {path} | "
            f"ckpt_epoch={self._ckpt_epoch} global_step={self.global_step}"
        )
    ...
    def train(self, dataloader: DataLoader) -> None:
        ...
        steps_per_epoch = len(dataloader) if len(dataloader) > 0 else 0

        # ---- FIXED RESUME LOGIC ----
        # Use ONLY global_step + current dataloader length
        # to figure out where to start.
        if self.global_step > 0 and steps_per_epoch > 0:
            start_epoch = self.global_step // steps_per_epoch
            batch_offset = int(self.global_step % steps_per_epoch)
            self._log(
                f"Continuing training from global_step={self.global_step} "
                f"(start_epoch={start_epoch}, batch_offset={batch_offset}, "
                f"steps_per_epoch={steps_per_epoch})"
            )
        else:
            start_epoch = 0
            batch_offset = 0
            self._log("Starting training from scratch.")

        # Cap by cfg.epochs: epochs is the TOTAL number of epochs we want.
        if start_epoch >= cfg.epochs:
            self._log(
                f"start_epoch ({start_epoch}) >= cfg.epochs ({cfg.epochs}); "
        ...
        for epoch in range(start_epoch, cfg.epochs):
            ...
            for batch_i, batch in enumerate(epoch_bar):
                # If resuming mid-epoch, skip batches until we reach batch_offset
                if epoch == start_epoch and batch_i < batch_offset:
                    continue
