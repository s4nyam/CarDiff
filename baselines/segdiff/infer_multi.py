...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--masks-dir',
        type=Path,
        default=Path('./MASK_FOLDER/all/test'),
        help='Directory of input segmentation masks.'
    )
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('./OUTPUT_FOLDER/DDPM'),
...
def main() -> None:
    ...
    # Wrap and move to device (no DataParallel)
    unet.to(device)
    unet.eval()

    # Detect seg-guided based on channel configuration
    try:
        in_ch = int(getattr(unet.config, "in_channels"))
        out_ch = int(getattr(unet.config, "out_channels", 1))
    except Exception:
        in_ch, out_ch = 2, 1
    ...
    if isinstance(sample_size, int):
        ...
        width = int(sample_size[0])
        height = int(sample_size[1])
