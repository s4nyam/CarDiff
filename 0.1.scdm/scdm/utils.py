...
def resolve_device(device_str: Optional[str] = None) -> torch.device:
    ...
    Accepts values such as "cpu", "cuda", "cuda:0", or "auto"/None to
    select the best available device. Raises a ValueError when the requested
    accelerator is unavailable or malformed.
    """
    ...
    except (TypeError, ValueError, RuntimeError) as exc:  # pragma: no cover - defensive
...
def mask_to_edge_map(mask: torch.Tensor) -> torch.Tensor:
    """Compute binary edge maps from segmentation masks."""
    if mask.dim() == 2:
        mask = mask.unsqueeze(0)
    if mask.dim() == 3:
        mask = mask.unsqueeze(1)
    if mask.dim() != 4:
        raise ValueError("Expected mask tensor of shape (B, 1, H, W)")
