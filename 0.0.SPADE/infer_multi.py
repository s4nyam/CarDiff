...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--checkpoint', type=Path, default=Path('./spade_runs_multi/checkpoints/spade_step_0652000.pt'), help='Path to a trained SPADE checkpoint (.pt).')
    parser.add_argument('--masks-dir', type=Path, default=Path('./test_masks_multi'), help='Directory of input segmentation masks.')
    parser.add_argument('--output-dir', type=Path, default=Path('./infer_outputs_multi'), help='Directory to write generated images.')
    parser.add_argument('--samples-per-mask', type=int, default=1, help='Number of images to sample per mask.')
    parser.add_argument('--device', type=str, default="cuda:1", help='"cpu" or "cuda". Auto if omitted.')
